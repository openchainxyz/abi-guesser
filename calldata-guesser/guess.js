...
const tryParseOffset = (data, pos) => {
    const bigOffset = BigInt(ethers_1.ethers.utils.hexlify(data.slice(pos, pos + 32)));
    // can't be huge
...
const tryParseLength = (data, offset) => {
    const bigLength = BigInt(ethers_1.ethers.utils.hexlify(data.slice(offset, offset + 32)));
    // can't be huge
...
dynamicOnlyParams) => {
    ...
    const testFragment = (fragment) => {
        if (!fragment)
            return false;
        try {
            abi_1.defaultAbiCoder.decode(fragment.inputs, data).map((v) => v.toString());
            return true;
        }
        catch (e) {
    ...
    if (paramOffset >= endOfStaticCalldata) {
        ...
        const dynamicParams = collectedParams.filter((v) => !abi_1.ParamType.isParamType(v));
        let dynamicParamIdx = -1;
        const finalParams = collectedParams.map((param) => {
            if (abi_1.ParamType.isParamType(param))
                return param; // not dynamic, no action
            ...
            const dynamicDataStart = dynamicParams[dynamicParamIdx].offset + (dynamicParams[dynamicParamIdx].length === null ? 0 : 32);
                ...
                if ((maybeDynamicElementLen === 0 && dynamicData.length === 0) ||
                    ...
                    if (dynamicData.length / 32 > maybeDynamicElementLen) {
                        ...
                        if (elementsHaveOffsets) {
                            ...
                            return abi_1.ParamType.from(`${Array.from(allResults)[0]}[]`);
                        }
                        else {
                            ...
                            if ((dynamicData.length / 32) % maybeDynamicElementLen !== 0) {
                                ...
                                dynamicData = dynamicData.slice(0, Math.floor(dynamicData.length / 32 / maybeDynamicElementLen) *
                            ...
                            for (let elemIdx = 0; elemIdx < maybeDynamicElementLen; elemIdx++) {
                                const fragment = decodeWellFormedTuple(depth + 1, dynamicData.slice(elemIdx * elemSizeWords * 32, (elemIdx + 1) * elemSizeWords * 32), 0, [], elemSizeWords * 32, null);
                                if (!fragment) {
            ...
            if (maybeDynamicElementLen !== null) {
                ...
                if (result.length === 1) {
                    if (result[0].type === 'bytes') {
                        ...
                        return abi_1.ParamType.from(`${result[0].format()}[]`);
                    }
                }
                return abi_1.ParamType.from(`(${result.map((v) => v.format()).join(',')})[]`);
            }
            return abi_1.ParamType.from(`(${inputs.map((v) => v.format()).join(',')})`);
        });
        const filteredParams = finalParams.filter((v) => v !== undefined);
        ...
        const fragment = lib_1.FunctionFragment.from(`guessed(${filteredParams.map(formatParams).join(', ')})`);
    ...
    if (paramIdx === 0) {
        debug('backtracking');
        debug('input:');
        chunkString(ethers_1.ethers.utils.hexlify(data).substring(2), 64).forEach((v, i) => debug('  ' + i.toString(16) + ' => ' + v));
    }
    ...
    if (maybeOffset !== null) {
        ...
        debug(`parameter ${paramIdx} might be dynamic, got offset ${maybeOffset}, len ${maybeLength}, ${depth}, ${dynamicOnlyParams}`);
        if (maybeLength !== null &&
            (dynamicOnlyParams === null || (depth === dynamicOnlyParams.depth && dynamicOnlyParams.assumeLength))) {
            const fragment = decodeWellFormedTuple(depth, data, paramIdx + 1, [...collectedParams, { offset: maybeOffset, length: maybeLength }], Math.min(endOfStaticCalldata, maybeOffset), dynamicOnlyParams);
            if (testFragment(fragment)) {
                return fragment;
            }
        }
        if (dynamicOnlyParams === null || (depth === dynamicOnlyParams.depth && !dynamicOnlyParams.assumeLength)) {
            const fragment = decodeWellFormedTuple(depth, data, paramIdx + 1, [...collectedParams, { offset: maybeOffset, length: null }], Math.min(endOfStaticCalldata, maybeOffset), dynamicOnlyParams);
            if (testFragment(fragment)) {
                return fragment;
            }
        }
    ...
    const fragment = decodeWellFormedTuple(depth, data, paramIdx + 1, [...collectedParams, abi_1.ParamType.from('bytes32')], endOfStaticCalldata, dynamicOnlyParams);
    if (testFragment(fragment)) {
        return fragment;
    }
    return null;
 ...
 */
const wellFormedParse = (sig, bytes) => {
    ...
    const prettyTypes = (params, vals) => {
        return params.map((param, idx) => {
            ...
            if (param.type === 'bytes32') {
                const leadingZeros = countLeadingZeros(ethers_1.ethers.utils.arrayify(val));
                const trailingZeros = countTrailingZeros(ethers_1.ethers.utils.arrayify(val));
                ...
                try {
                    ethers_1.ethers.utils.toUtf8String(val);
    ...
    return lib_1.FunctionFragment.from(`guessed_${ethers_1.ethers.utils.hexlify(sig).substring(2)}(${prettyTypes(fragment.inputs, Array.from(abi_1.defaultAbiCoder.decode(fragment.inputs, bytes)))
        .map((v) => v.format())
        .join(',')})`);
};
const guessFragment = (calldata) => {
    const bytes = ethers_1.ethers.utils.arrayify(calldata);
    if (bytes.length === 0)
        return null;
