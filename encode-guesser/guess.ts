...
const decodeWellFormedTuple = (
    ...
    if (paramOffset < endOfStaticCalldata) {
        ...
        if (maybeOffset !== null) {
            ...
            if (
                maybeLength !== null && (isDynamicArrayElement === null || isDynamicArrayElement === true)
            ) {
    ...
    const maybeResolveDynamicParam = (idx: number): ParamType | undefined => {
        ...
        const nextDynamicParam = collectedParams.find((v, i): v is DynamicPlaceholder => i > idx && !ParamType.isParamType(v));
 ...
 */
export const guessAbiEncodedData = (bytes: Uint8Array): ParamType[] | null => {
...
export const guessFragment = (calldata: BytesLike): FunctionFragment | null => {
    ...
    return FunctionFragment.from(
        `guessed_${hexlify(selector).substring(2)}(${prettyTypes(
            params,
            Array.from(defaultAbiCoder.decode(params, tupleData)),
        )
            .map((v) => v.format())
            .join(',')})`,
    );
